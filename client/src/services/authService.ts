...
export const Registration = async (username: string) => {
    try {
        const registrationStartResponse = await axiosInstance.post(`api/auth/start_reg/${username}`);
        ...
        const registrationFinishResponse = await axiosInstance.post(`api/auth/finish_reg`, JSON.stringify(attResponse));
...
export const Authentication = async (username: string) => {
    try {
        const authenticationStartResponse = await axiosInstance.post(`api/auth/start_auth/${username}`);
        ...
        const authenticationFinishResponse = await axiosInstance.post(`api/auth/finish_auth/${username}`, attResp);
